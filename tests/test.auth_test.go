...
var routerAuth = configs.NewRouterTesting()
var accessToken string

func TestPingHandler(t *testing.T) {

	rr, req, _ := helpers.HttpTestRequest(http.MethodGet, "/api/v1/ping", nil)
	routerAuth.ServeHTTP(rr, req)

	res := rr.Result()
	parse := helpers.Parse(rr.Body.Bytes())

	assert.Equal(t, res.StatusCode, 200)
	assert.Equal(t, parse.Message, "Ping Test")
}
...
func TestRegisterHandler(t *testing.T) {
	...
	Convey("Auth Register Handler Group", t, func() {
		...
		Convey("Register New Account", func() {
			...
			test := supertest.NewSuperTest(routerAuth, t)
